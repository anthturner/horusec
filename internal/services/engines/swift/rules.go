...
func NewDTLS11NotUsed() text.TextRule {
	return text.TextRule{
		Metadata: engine.Metadata{
			ID:          "HS-SWIFT-3",
			Name:        "DTLS 1.0 not used",
		...
		Type: text.Regular,
		Expressions: []*regexp.Regexp{
			regexp.MustCompile(`tls_protocol_version_t.DTLSv11`),
...
func NewTLS13NotUsed() text.TextRule {
	return text.TextRule{
		...
		Type: text.Regular,
		Expressions: []*regexp.Regexp{
			regexp.MustCompile(`tls_protocol_version_t.TLSv11`),
			regexp.MustCompile(`tls_protocol_version_t.TLSv10`),
		},
	}
}
